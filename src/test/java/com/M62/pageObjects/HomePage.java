...
public class HomePage {
	...
	public HomePage SelectFrom(final String Country) {
		...
		FromCountry = (new WebDriverWait(driver, 10))
	...
	public HomePage SelectFromAirport(final String Airport) {
		...
		WebElement FromAirport = (new WebDriverWait(driver, 10))
				.until(new ExpectedCondition<WebElement>() {
					public WebElement apply(WebDriver d) {
	...
	public HomePage SelectToCountry(final String Country) {
		...
		((JavascriptExecutor) driver).executeScript("window.scrollBy(0,250)");
		ToCountry = (new WebDriverWait(driver, 10))
				.until(new ExpectedCondition<WebElement>() {
					public WebElement apply(WebDriver d) {
						return d.findElement(By.xpath("//*[text()='" + Country
								+ "']"));
	...
	public HomePage SelectToAirport(final String Airport) {
		...
		ToAirport = (new WebDriverWait(driver, 10))
				.until(new ExpectedCondition<WebElement>() {
					public WebElement apply(WebDriver d) {
						return d.findElement(By.xpath("//*[text()='" + Airport
								+ "']"));
	...
	public String getSelectedToValue() {
		new WebDriverWait(driver, 10).until(ExpectedConditions
				.textToBePresentInElementValue(To, "Manchester"));
	...
	public HomePage SelectFromCalendar(String StartDate, String startMonth) {
		...
		while (i < 11) {
			WebElement Month = driver.findElement(By
					.xpath("(//h1[@class='month-name'])[" + i + "]"));
			if (Month.getText().toLowerCase()
					.contains(startMonth.toLowerCase())) {
				WebElement Calendar = driver.findElement(By
						.xpath("(//core-datepicker//ul[@class='days'])[" + i
								+ "]"));
				List<WebElement> Days = Calendar.findElements(By
						.xpath("(//core-datepicker//ul[@class='days'])[" + i
								+ "]/li"));
				for (WebElement day : Days) {
					String str = day.getAttribute("data-id").substring(0, 2);
					if ((str.substring(0, 1).equals("0") ? str.substring(1)
							: str).equals(StartDate)) {
						day.click();
						return this;
					}
				}
			} else {
				driver.findElement(By.xpath("//button[@class='arrow right']"))
						.click();
				i++;
			}
		}
		...
		TeenPlusButton = wait.until(new Function<WebDriver, WebElement>() {
			public WebElement apply(WebDriver driver) {
				return driver.findElement(By
						.xpath("(//*[@icon-id='glyphs.plus-circle'])[2]/.."));
			}
		});
	...
	public SearchResultPage Search() {
		...
		return StaticFactoryClass.initializeSearchResultsPage(driver);
	}
